...
class ManualFleetSync:
    ...
    def get_github_token(self) -> str:
        """Lấy GitHub token từ user input hoặc file"""
        ...
        # Nếu chưa có, hỏi user (chỉ khi chạy standalone)
        if __name__ == "__main__":
            print("🔑 GITHUB TOKEN SETUP")
            print("=" * 40)
            print("1. Vào: https://github.com/settings/tokens")
            print("2. Tạo new token (classic)")
            print("3. Chọn scopes: repo, user, public_repo")
            print("4. Copy token và paste vào đây:")
            print()
            
            token = input("Paste GitHub token: ").strip()
            
            if token:
                # Lưu token để lần sau
                try:
                    with open(token_file, 'w') as f:
                        f.write(token)
                    print("✅ Đã lưu token!")
                except:
                    print("⚠️ Không thể lưu token, sẽ hỏi lại lần sau")
